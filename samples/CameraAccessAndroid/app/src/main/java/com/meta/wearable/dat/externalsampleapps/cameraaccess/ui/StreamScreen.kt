...
import com.meta.wearable.dat.externalsampleapps.cameraaccess.cvp.BurstState
...
fun StreamScreen(
    ...
    val burstState by streamViewModel.burstController.state.collectAsStateWithLifecycle()
    val isPipelineReady by streamViewModel.isPipelineReady.collectAsStateWithLifecycle()
    val lifecycleOwner = LocalLifecycleOwner.current
    val context = LocalContext.current

    // Wire Gemini VM to Stream VM for frame forwarding + speech-triggered capture
    LaunchedEffect(geminiViewModel) {
        streamViewModel.geminiViewModel = geminiViewModel
        geminiViewModel.onSpeechStarted = {
            if (SettingsManager.workModeEnabled) streamViewModel.triggerLook()
        }
        geminiViewModel.onProactiveCapture = {
            if (SettingsManager.workModeEnabled) streamViewModel.triggerLook()
        }
    }
